...
typedef struct dladm_vnic_attr {
	...
	vnic_mac_addr_type_t	va_mac_addr_type_src;
	uint_t			va_mac_len;
	...
	uint_t			va_alt_naddrs;
	vnic_mac_addr_t		*va_alt_addrs;
	uint_t			va_alt_nvids;
	uint16_t		*va_alt_vids;
	mac_iov_pref_t		va_iov;
	char			va_vf_assigned[MAXLINKNAMELEN];
	uint_t			va_guest_owned;
	char			va_devpath[MAXPATHLEN];
	boolean_t		va_evsvnic;
	boolean_t		va_shadowvnic;
	mac_lro_t		va_lro;
	mac_ib_props_t		va_ibprops;
} dladm_vnic_attr_t;
#endif
...
extern dladm_status_t	dladm_vnic_delete(dladm_handle_t, datalink_id_t,
			    uint32_t);*/
/*extern int		dladm_vnic_info(dladm_handle_t, datalink_id_t,
