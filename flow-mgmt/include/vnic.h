...
//#define	VNIC_IOC_INFO		0x1710003
...
typedef struct vnic_info {
	...
	vnic_mac_addr_type_t vn_mac_addr_type_src;
	uint_t		vn_mac_len;
	...
	int		vn_af;
    ib_pkey_t vn_pkey;
    mac_iov_pref_t vn_iov;
    boolean_t vn_shadowvnic;
    boolean_t vn_evsvnic;
    vnic_uconf_flags_t vn_uconf_flags;
    char vn_devpath[1024];
    char vn_vf_assigned[32];
    uint_t vn_alt_naddrs;
    uint_t vn_alt_nvids;
} vnic_info_t;
...
vnic_info_t {
    ...
    vnic_mac_addr_type_t vn_mac_addr_type_src
    uint_t vn_mac_len
    uchar_t [20] vn_mac_addr
    uint_t vn_mac_slot
    uint32_t vn_mac_prefix_len
    vnic_vlan_t vn_vlan {
        uint16_t vv_vid
        uint16_t vv_pvlan_svid
        mac_pvlantype_t vv_pvlan_type
    }
    vrid_t vn_vrid
    int vn_af
    ib_pkey_t vn_pkey
    ...
    boolean_t vn_evsvnic
    vnic_uconf_flags_t vn_uconf_flags {
        unsigned vc_guest_owned :1
        unsigned vc_vlan_force :1
        unsigned vc_tag_disable :1
        unsigned vc_strip_disable :1
        unsigned vc_nodupchk :1
        unsigned vc_portvlan_sw :1
        unsigned vc_ipoib_force :1
        unsigned vc_pad_to_bit31 :25
    }
    char [1024] vn_devpath
    char [32] vn_vf_assigned
    uint_t vn_alt_naddrs
    uint_t vn_alt_nvids
}
